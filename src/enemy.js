...
export class Enemy {
    ...
    _applyStep(dx, dy) {
        ...
        for (const w of this.world.wallsInRect(nx - near, ny - near, near * 2, near * 2)) {
            if (circleOverlapsWall(nx, ny, ENEMY_RADIUS, w))
                return;
        }
    ...
    _checkLOS(player) {
        ...
        if (Math.hypot(dx, dy) > VISION_DIST)
            return false;
        ...
        while (diff < -Math.PI)
            diff += 2 * Math.PI;
        if (Math.abs(diff) > VISION_HALF)
            return false;
        const walls = this.world.wallsForSegment(this.x, this.y, player.x, player.y);
        if (!hasLineOfSight(this.x, this.y, player.x, player.y, walls))
            return false;
        if (this.world.treesForSegment(this.x, this.y, player.x, player.y)
            .some(t => treeBlocksSegment(this.x, this.y, player.x, player.y, t)))
            return false;
        if (this.world.rocksForSegment(this.x, this.y, player.x, player.y)
            .some(r => rockBlocksSegment(this.x, this.y, player.x, player.y, r)))
            return false;
        return true;
    }
    draw(ctx) {
    ...
    _drawCone(ctx) {
        ...
        ctx.arc(this.x, this.y, VISION_DIST, this.facing - VISION_HALF, this.facing + VISION_HALF);
        ctx.closePath();
    ...
    _drawBody(ctx) {
        ...
        ctx.beginPath();
        ctx.moveTo(this.x, this.y);
        ctx.lineTo(this.x + Math.cos(this.facing) * 15, this.y + Math.sin(this.facing) * 15);
