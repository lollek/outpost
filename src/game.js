import { World, W, H, spawnPoint } from './world';
import { Player, HAMMER_COST, WALL_LEN, WALL_T } from './player';
import { Enemy } from './enemy';
import { wallEndpoints } from './los';
...
function loop(ts) {
    ...
    drawHUD(ctx);
    if (craftingOpen)
        drawCraftingPanel(ctx);
    requestAnimationFrame(loop);
}
function drawGhostWall(ctx, wall, valid) {
...
function drawHammerIcon(ctx, x, y, color) {
    ctx.save();
    ctx.translate(x, y);
