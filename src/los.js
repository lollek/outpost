...
export function aabbWall(x, y, w, h) {
...
export function wallLineAngle(wall) {
 ...
 */
function wallBlocksSegment(ox, oy, tx, ty, wall) {
    const c = wallCorners(wall);
    for (let i = 0; i < 4; i++) {
        const p = c[i], q = c[(i + 1) % 4];
        if (segmentsIntersect(ox, oy, tx, ty, p.x, p.y, q.x, q.y))
            return true;
    }
    return false;
}
...
export function hasLineOfSight(ox, oy, tx, ty, walls) {
    for (const wall of walls) {
        if (wallBlocksSegment(ox, oy, tx, ty, wall))
            return false;
    }
    return true;
}
 ...
 */
export function circleOverlapsWall(px, py, r, wall) {
    const cos = Math.cos(wall.a), sin = Math.sin(wall.a);
...
export function wallsOverlap(a, b) {
    ...
    for (const ax of axes) {
        ...
        if (amax <= bmin + EPS || bmax <= amin + EPS)
            return false; // separated or just touching
    }
    return true;
}
/**
 ...
 */
export function segmentIntersectsCircle(ox, oy, tx, ty, cx, cy, r) {
    ...
    if (a < 1e-12)
        return c <= 0; // degenerate segment: is the point inside?
