...
export function findPath(sx, sy, tx, ty, world) {
    ...
    const cellBlocked = (col, row) => {
        if (col < 0 || col >= cols || row < 0 || row >= rows)
            return true;
        const cx = originX + col * CELL + CELL / 2;
        const cy = originY + row * CELL + CELL / 2;
        for (const w of walls) {
            if (circleOverlapsWall(cx, cy, CLEARANCE, w))
                return true;
        }
        for (const t of trees) {
            if (Math.hypot(cx - t.x, cy - t.y) < CLEARANCE + TREE_RADIUS)
                return true;
        }
        for (const rk of rocks) {
            if (Math.hypot(cx - rk.x, cy - rk.y) < CLEARANCE + rk.r)
                return true;
        }
        return false;
    };
    ...
    while (open.length > 0) {
        ...
        for (let i = 1; i < open.length; i++) {
            if (fScore[open[i]] < fScore[open[bestAt]])
                bestAt = i;
        }
        ...
        for (const [dc, dr, cost] of DIRS) {
            ...
            if (dc !== 0 && dr !== 0) {
                if (cellBlocked(cc + dc, cr))
                    continue;
                if (cellBlocked(cc, cr + dr))
                    continue;
            }
